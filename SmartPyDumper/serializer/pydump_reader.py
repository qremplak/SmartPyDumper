...
class PydumpUnitReader:
    ...
    def __init__(self, path_or_object):
        if is_pydump_unit_path(path_or_object):
            self.path=path_or_object
            self.var_name=Path(path_or_object).stem
        else:
            self.var_name=''
            self.path=None
        
        self.type_data=get_str_type_from_unit(path_or_object)
        self.data=deserialize_object(path_or_object)
